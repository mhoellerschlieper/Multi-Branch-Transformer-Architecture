...
// - 2026-02-07: Add ParallelBlockGroup diagnostics metrics after checkpoint load.
...
// ---------------------------
// Metrics computed over a fixed evaluation batch, using per-branch outputs
// for a given input activation X to the ParallelBlockGroup.
// The design uses only ASCII identifiers and safe numeric handling.
//
// Metrics list:
// - d_path_starvation_index: 1 - normalized entropy of per-branch contribution distribution
// - d_diversity_cosine_distance_mean: mean pairwise cosine distance between branch outputs
// - d_effective_num_paths: exp(entropy) (perplexity style), in [1..k]
// - d_gini_concentration: gini coefficient of contribution distribution
// - d_top1_share: max(p_i)
// - d_margin_top1_top2: p_top1 - p_top2
// - d_output_energy_cv: coefficient of variation of branch output L2 energies
// - d_branch_correlation_mean: mean pairwise cosine similarity (redundancy indicator)
#[derive(Clone, Debug)]
...
impl ParallelBlockGroupMetrics {
    pub fn to_ascii_report_lines(&self) -> Vec<String> {
        ...
        v.push(format!(
            "path_starvation_index: {:.6}",
            self.d_path_starvation_index
        ));
        v.push(format!(
        ...
        v.push(format!(
            "effective_num_paths: {:.6}",
            self.d_effective_num_paths
        ));
        v.push(format!(
            "gini_concentration: {:.6}",
            self.d_gini_concentration
        ));
        v.push(format!("top1_share: {:.6}", self.d_top1_share));
        v.push(format!(
            "margin_top1_top2: {:.6}",
            self.d_margin_top1_top2
        ));
        v.push(format!(
            "output_energy_cv: {:.6}",
            self.d_output_energy_cv
        ));
        v.push(format!(
...
// Compute entropy of distribution p (assumes p sums approx to 1, non-negative)
...
// Normalize distribution to sum to 1; if degenerate, return uniform.
fn normalize_distribution(v_x: &[f32]) -> Vec<f32> {
    ...
    for &d in v_x.iter() {
        let d_v = sanitize_f32(d).max(0.0);
        d_sum += d_v;
    }
...
// Gini coefficient for non-negative distribution p that sums to 1.
// For sorted p ascending: G = 1 - 2 * sum_i p_i * (n - i + 0.5) / n
...
// Cosine similarity between two vectors; safe fallback to 0.0.
...
// Flatten an Array2 into Vec<f32> row-major; safe.
fn flatten_array2(a_x: &Array2<f32>) -> Vec<f32> {
    a_x.iter().map(|&d| sanitize_f32(d)).collect()
}

// L2 energy (mean square) of an array; safe.
...
// Coefficient of variation: std / mean; safe.
...
pub struct ParallelBlockGroup {
    ...
    // Fault injection controls (test-only):
    // - When enabled, one branch index can be skipped during forward aggregation.
...
impl ParallelBlockGroup {
    ...
    // Set which branch is dropped. None disables dropping even if enabled.
    pub fn set_fault_drop_branch_idx(&mut self, opt_idx: Option<usize>) {
        if let Some(i_idx) = opt_idx {
            if i_idx >= self.v_branches.len() {
                // Invalid index -> treat as no drop to keep safe behavior.
                self.opt_fault_drop_branch_idx = None;
                return;
            }
    ...
    // Compute per-branch outputs for diagnostics without changing aggregation.
    // This runs forward on each branch; caller should set model to eval mode if needed.
    ...
    // Compute metrics using a fixed batch of inputs to this group.
    // v_inputs: list of activations shaped [seq, emb] as expected by the group.
    pub fn compute_metrics_from_inputs(
        ...
        for a_in in v_inputs.iter() {
            ...
            // Branch score: mean square energy of output.
            // This is a target-free utilization proxy suitable right after load.
            let mut v_scores: Vec<f32> = Vec::with_capacity(i_k);
            let mut v_energy: Vec<f32> = Vec::with_capacity(i_k);

            // Also cache flattened vectors for pairwise metrics.
            ...
            // Convert scores to probability distribution p via softmax over scores.
            ...
            // 1) Path starvation index (PSI) = 1 - H(p)/ln(k)
            ...
            // 3) Effective number of paths = exp(H(p))
            ...
            // 2) Diversity: mean pairwise cosine distance of outputs.
            // 8) Correlation mean: mean pairwise cosine similarity (redundancy).
            ...
            // 7) Output energy CV aggregated later across samples as well.
...
impl Layer for ParallelBlockGroup {
    ...
    fn forward(&mut self, a_input: &Array2<f32>) -> Array2<f32> {
        ...
        // Sum outputs of non-dropped branches.
        ...
        // Re-normalize weighting based on actually used branches.
        // This ensures stable amplitude even if a branch is dropped.
    ...
    fn backward(&mut self, a_grads: &Array2<f32>, d_lr: f32) -> Array2<f32> {
        // NOTE:
        // Fault injection is specified "before each predict" (eval/inference).
        // Backward is kept conservative and does not simulate outage by default.
        // This avoids unintended training artifacts.
        if a_grads.nrows() == 0 || a_grads.ncols() == 0 {
            return a_grads.clone();
        }
